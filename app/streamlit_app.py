...
def safe_dataframe_preview(file_path: Path) -> pd.DataFrame:
    ...
    try:
        preview_rows = get_int_config("dataset.profiling_sample_rows", 100_000)
...
def remove_non_serializable_objects(result: dict[str, Any]) -> dict[str, Any]:
    ...
    baseline_raw = clean_result.get("baseline_results", {})
    ...
    st.markdown(
        ...
        <div class="hero">
            ...
            <p>
                ...
                baseline models, MLflow tracking, feature importance, SHAP summaries,
...
def show_upload_panel() -> None:
    ...
    max_upload_mb = get_float_config("streamlit.max_upload_mb", 25.0)
...
def show_metric_recommendation(metric_result: dict[str, Any]) -> None:
    ...
    c2.metric("Sklearn Scoring", metric_result.get("scoring_metric", "N/A"))
...
def show_human_review(human_review: dict[str, Any]) -> None:
    ...
    if review_items:
        ...
        if "severity" in review_df.columns:
            ...
            review_df = review_df.sort_values("severity_rank").drop(
