...
def safe_int(value: Any, default: int) -> int:
    """Convert config values to int with a safe fallback."""
    try:
        return int(value)
...
def get_log_level() -> int:
    ...
    level_name = str(get_config_value("logging.log_level", "INFO")).strip().upper()
...
def add_file_handler(
    ...
    log_file = str(get_config_value("logging.log_file", _DEFAULT_LOG_FILE))
    max_bytes = safe_int(
        get_config_value("logging.max_bytes", _DEFAULT_MAX_BYTES),
        _DEFAULT_MAX_BYTES,
    )
    backup_count = safe_int(
        get_config_value("logging.backup_count", _DEFAULT_BACKUP_COUNT),
...
def _update_existing_handlers(
    ...
    """Update existing handler levels and formatter during reruns/tests."""
...
def get_logger(name: str) -> logging.Logger:
    ...
    Supports config-driven log level, optional console/file logging,
    rotating file logs, duplicate handler prevention, and safe fallback.
    """
    ...
    if logger.handlers:
        _update_existing_handlers(logger, log_level, formatter)
        return logger
    ...
    if file_logging:
        try:
            ...
            # Console logging can still provide visibility.
            pass
